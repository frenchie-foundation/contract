...
//
//                   Frenchie.so / Frenchie.tech
//
// Supported by #FrenchieNetwork t.me/FrenchieNetwork
...
contract FrenchieDAO is ERC20 {
  ...
  mapping(address => frenVoted) private frenVotes;
...
/// @notice The EIP-712 typehash for the contract's domain
  ...
  constructor(address _frenddress) public ERC20('FrenchieDAO', 'FrenDAO') {
    currentAdmins[msg.sender].isAdmin = true;
    frenAddress = _frenddress;
    frenIERC20 = IERC20(frenAddress);
  }

  function getFrenBalance() public view returns (uint256) {
    return ERC20Interface(frenAddress).balanceOf(msg.sender);
  }
  ...
  function setFrenAddress(address _addr) public _isAdmin {
    frenAddress = _addr;
    frenIERC20 = IERC20(frenAddress);
  }
  ...
  function voteForProposal(
    ...
    frenIERC20.safeTransferFrom(msg.sender, address(this), _amount);

    frenVotes[msg.sender].votes = frenVotes[msg.sender].votes.add(_amount);
    frenVotes[msg.sender].proposalAddr = _proposalAddr;
    frenVotes[msg.sender].proposalQuestion = eachProposal[_proposalAddr]
      .proposalQuestion;
    frenVotes[msg.sender].option = eachProposal[_proposalAddr]
    ...
    emit votedForProposal(
      ...
      frenVotes[msg.sender].option
    );
  }

  function withdrawFren() public {
    require(
      block.number > eachProposal[proposals[0]].endBlock,
      'Voting has not ended yet!'
    );
    address propAddr = frenVotes[msg.sender].proposalAddr;
    string memory propQuestion = frenVotes[msg.sender].proposalQuestion;
    ...
    uint256 amount = frenVotes[msg.sender].votes;
    frenIERC20.safeTransfer(msg.sender, amount);
    frenVotes[msg.sender].votes = frenVotes[msg.sender].votes.sub(amount);
    emit frenWithdrawn(msg.sender, amount);
  }
