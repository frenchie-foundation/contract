//  _____ ____  _____ _   _  ____ _   _ ___ _____   _____ _    ____  __  __ 
// |  ___|  _ \| ____| \ | |/ ___| | | |_ _| ____| |  ___/ \  |  _ \|  \/  |
// | |_  | |_) |  _| |  \| | |   | |_| || ||  _|   | |_ / _ \ | |_) | |\/| |
...
// File contracts/frenchielpfarm.sol
...
/**
 * 50 billion FREN tokens are allocated as incentive for liquidity providers on uniswap.
 * This incentive will be distributed on an approx. weekly-basis over 3 months (13 weeks) 
 * Liquidity providers (LPs) can deposit their LP-tokens (UniswapV2Pair token for FREN-BSC)
 * to this FrenFarm contract for at least 1 week (minimum deposit period) to receive rewards. 
 */
contract FrenchieFarm is IERC777Recipient, ReentrancyGuard {
    ...
    modifier onlyMultisig(address adr) {
        require(adr == multisig, "FrenFarm: Only DAO multisig");
        _;
    }

    /**
     * @dev provides the farming schedule.
     * @param _pool address Address of the  Pancake FREN-BSC uniswap pool.
     * @param _token address Address of the FREN token.
    ...
    /**
     * @dev ERC777 hook triggered when multisig send FREN token to this contract.
    ...
    function tokensReceived(
        ...
        require(msg.sender == address(hopr), "FrenFarm: Sender must be FREN token");
        require(to == address(this), "FrenFarm: Must be sending tokens to FREN farm");
        require(amount == TOTAL_INCENTIVE, "FrenFarm: Only accept 5 million FREN token");
        // take block number from userData, varies from 0x000000 to 0xffffff.
        // This value is sufficient as 0xffff will be in March 2023.
        require(userData.length == 3, "FrenFarm: Start block number needs to have three bytes");
        require(distributionBlocks[0] == 0, "FrenFarm: Not initialized yet.");
        bytes32 m;
        assembly {
        ...
        require(startBlock >= block.number, "FrenFarm: Start block number should be in the future");
    ...
    /**
     * @dev liquidity provider deposits their Pancake FREN-BSC tokens to the contract
    ...
    /**
     * @dev Called by liquidty provider to deposit their Pancake FREN-BSC tokens to the contract
     * It updates the current balance and the eligible farming balance
    ...
    /**
     * @dev liquidity provider deposits their Pancake FREN/BSC LP tokens to the contract
     * It updates the current balance and the eligible farming balance
     * @param amount Amount of pool token to be staked into the contract.
